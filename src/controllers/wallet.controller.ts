...
export const getBalance = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const orders = await prisma.order.findMany({ where: { userId, type: 'buy' } });

    // Calcular valor total de monedas compradas
    const coins = Coin.getCoins();
    const coinDetails: Record<string, { amount: number; value: number; currentPrice: number }> = orders.reduce((acc, order) => {
      const coin = coins.find(c => c.symbol.toLowerCase() === order.symbol.toLowerCase());
      if (coin) {
    ...
    const totalCoinValue = Object.values(coinDetails).reduce((sum, coin) => sum + coin.value, 0);
